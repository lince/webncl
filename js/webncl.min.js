...
e;this.presentation.keys.allCodes=[13,37,38,39,40,81,87,69,82,96,97,98,99,100,101,102,103,104,105,90,88,67,86,66,78,65,83,68];this.presentation.postEvent=$.proxy(this.postEvent,this);this.presentation.isPlaying=!1;this.presentation.isStopped=!0;this.presentation.playingElem=[];this.presentation.pausedElem=[];this.presentation.notifyLink=$.proxy(this.nAction,this);this.presentation.lastMediaAborted=!1;this.presentation.parser=new Parser(this.presentation.path);a?this.presentation.parser.load(a,$.proxy(this.execute,
this)):(a=document.URL.split("/"),a=a[a.length-1],this.presentation.parser.load(a,$.proxy(this.execute,this),this.div))}
...
InputManager.prototype.setCurrentFocus=function(a){if(void 0==a)this.currentFocusIndex=void 0;else if(-1!=this.focusIndexArray.indexOf(a)){if(this.currentFocusIndex){var b=this.descriptors[this.currentFocusIndex],c;for(c in b.mediaArray)b=currentDescriptor.mediaArray[c],this.removeMediaFocus(b)}currentDescriptor=this.descriptors[a];for(c in currentDescriptor.mediaArray)b=currentDescriptor.mediaArray[c],this.setMediaFocus(b);this.currentFocusIndex=a;return!0}return!1};
...
Parser.prototype.importNcl=function(a){if(a)for(var b in a.parsers)this.parsers[[a.alias,"#",b].join("")]=a.parsers[b];a=this.uniqueTable.aliasList.splice(0,1);0<a.length?a[0].parser.load(this.path+a[0].url,$.proxy(this.importNcl,this)):(a=new Date,this.referenceMap.createReferences(),a=new Date-a,this.info.parseTime+=a,this.__callback&&this.__callback(this))};
...
c,[g])};function ReferenceMap(a){this.map={};this.instReuse=[];this.parser=a}ReferenceMap.prototype.addSource=function(a,b,c){var d=a[b];this.map[d]||(this.map[d]={sources:[]});this.map[d].sources.push({obj:a,attr:b,type:c});-1!==d.indexOf("#")&&(a=d.split("#"),this.map[d].target||(this.map[d].target={alias:a[0],id:a[1],obj:void 0,type:void 0,parents:[]}))};
...
ContextPlayer.prototype.bindLinks=function(){var a=this.node.port,b=this.node.link,c=this.handlers,d;for(d in a){var e=a[d],f="#"+this.presentation.getDivId(e.component.id),g=e.id,k;for(k in this.eventType){var h=this.eventType[k];if("attribution"!=h){var n=[h,k].join(".");$(f).on(n,{contextElement:this.htmlPlayer,interfaceId:g,eventName:n,portInterface:e["interface"]},function(a,b){if(b==a.data.portInterface){var c=$.Event(a.data.eventName);c.which=a.which;$(a.data.contextElement).trigger(c,[a.data.interfaceId])}})}}}for(d in b){var a=
b[d],m=a.xconnector,f={},g={},n=0,r="par",o=m.simpleCondition,s={},t={},l=[];if(m.connectorParam)for(d in m.connectorParam)s["$"+m.connectorParam[d].name]="";if(m.compoundCondition)for(d in o=m.compoundCondition[0].simpleCondition,1<o.length&&(n=Listener.listenerType[m.compoundCondition[0].operator.toUpperCase()]),m.compoundCondition[0].assessmentStatement){var j=m.compoundCondition[0].assessmentStatement[d];for(k in j.attributeAssessment)h=j.attributeAssessment[k],"attribution"==h.eventType&&(!h.attributeType||
"nodeProperty"==h.attributeType?t[h.role]={bindComponent:"",bindInterface:"",operator:j.comparator,value:j.valueAssessment.value,variable:ContextPlayer.isVariable(j.valueAssessment.value)}:console.warn("Eventos de tipos diferentes de attribution nao foram implementados para elementos do tipo AttributeAssessment"))}for(k in t)l.push(t[k]);for(d in o){var j=o[d],h=!1,e="",q=!1;"onSelection"===j.role&&j.key&&(ContextPlayer.isVariable(j.key)&&(h=!0),e=j.key,q=!0);g[j.role]={bindComponent:"",bindInterface:null,
eventType:this.eventType[j.role],flag:!1,keyUseVariable:h,keyDefaultValue:e,useKey:q}}o=m.simpleAction;m.compoundAction&&(r=m.compoundAction[0].operator.toLowerCase(),o=m.compoundAction[0].simpleAction);for(d in o)j=o[d],"set"==j.role?(m=!1,h="",j.value&&(ContextPlayer.isVariable(j.value)&&(m=!0),h=j.value),f[j.role]={binds:[],qualifier:o[d].qualifier,actionUseVariable:m,actionDefaultValue:h,pendingAcks:0}):f[j.role]={binds:[],qualifier:o[d].qualifier};m=[this];r=new Listener(n,r,f,g,l,this.presentation,
m);if(a.linkParam)for(d in a.linkParam)currentLinkParam=a.linkParam[d],s["$"+currentLinkParam.name]=currentLinkParam.value;for(d in a.bind)if(l=a.bind[d],q=l.component.id,n="#"+this.presentation.getDivId(q),o=l["interface"],f[l.role])h=null,j=f[l.role],"set"==l.role&&(j.actionUseVariable?(p=ContextPlayer.createLocalParamMap(l.bindParam),h=p[j.actionDefaultValue]?p[j.actionDefaultValue]:s[j.actionDefaultValue]):h=j.actionDefaultValue),this.context[q]&&o&&m.push(this.context[q]),j.binds.push({bindComponent:n,
bindInterface:o,value:h});else if(g[l.role]){e=g[l.role];e.bindComponent=n;e.bindInterface=o;var j=n,h=this.eventType[l.role],n=l.role,h=[h,n].join("."),q=[q,h].join("."),u=h+".triggerArray";if(e.useKey){var p=ContextPlayer.createLocalParamMap(l.bindParam);e.keyDefaultValue=p[e.keyDefaultValue]||s[e.keyDefaultValue];p=e.keyDefaultValue;$.isNumeric(p)&&(p="KEY_"+p);p in this.presentation.keys&&(e.keyDefaultValue=this.presentation.keys[p]);p="undefined";e=j;if("context"==l.component._type&&o){for(d in l.component.port)if(e=
l.component.port[d],portId=l.component.port[d].id,portId==o){p=e;break}e="#"+this.presentation.getDivId(p.component.id)}this.presentation.keyEvents[e]=!1}c[q]||(c[q]=!0,$(j).data(u)||$(j).data(u,[]),$(j).data(h)||$(j).data(h,[]),$(j).on(h,{componentDivId:j,bindName:h,eventName:n,triggerArrayName:u},function(a,b,c){var d=$(a.data.componentDivId).data(a.data.bindName),e;for(e in d){var f=d[e],g=f.flagMap[a.data.eventName];b==g.bindInterface&&(a.type=="selection"&&g.useKey?g.useKey&&a.which&&g.keyDefaultValue==
a.which&&f.notifyEvent(a.data.eventName):f.notifyEvent(a.data.eventName))}ContextPlayer.executeTriggerArray($(a.data.componentDivId).data(a.data.triggerArrayName));c&&c()}));r.triggerBuffer=$(j).data(u);$(j).data(h).push(r)}else currentAssessment=t[l.role],currentAssessment.bindComponent=n,currentAssessment.bindInterface=o,currentAssessment.variable&&(p=ContextPlayer.createLocalParamMap(l.bindParam),currentAssessment.value=p[currentAssessment.value]?p[currentAssessment.value]:s[currentAssessment.value])}};function FlowPlayer(a){this.p=a;this.htmlPlayer="#"+a.id;this.flowPlayer=void 0;a.createElement("<div class='player' id='"+a.id+"'></div>")}FlowPlayer.prototype.unload=function(){$(this.htmlPlayer).empty()};
...
"media",[a.source.type])}}if(a.checkType(["video","audio"])){do this.popcornPlayer=new Popcorn(this.htmlPlayer);while(!this.popcornPlayer)}else if(a.checkType(["image","text"])){do Popcorn.player("baseplayer"),this.popcornPlayer=new Popcorn.baseplayer(this.htmlPlayer);while(!this.popcornPlayer)}this.playing=!1;this.playingArea=void 0;this.anchors=[];for(i in a.area){var b=a.area[i].id;this.anchors[b]=a.area[i];this.anchors[b].temporal=a.area[i].begin||a.area[i].end||a.area[i].first||a.area[i].last?
!0:!1;this.anchors[b].textual=a.area[i].beginText||a.area[i].endText||a.area[i].beginPosition||a.area[i].endPosition?!0:!1;this.anchors[b].spatial=a.area[i].coords?!0:!1;this.anchors[b].temporal&&(eval("this.exec(this.anchors[id].beginTime,$.proxy(function() {$(this.htmlPlayer).trigger('presentation.onBegin',['"+b+"']);},this));"),eval("this.exec(this.anchors[id].endTime,$.proxy(function() {this.stopArea('"+b+"');},this));"));if(this.anchors[b].spatial&&!this.anchors[b]._ignore){var c=this.anchors[b].coords.split(",");
for(i in c){var d=c[i].split("%");1<d.length&&(c[i]='Math.round(parseInt($(this.htmlPlayer).css("'+(i%2?"height":"width")+'").split("px")[0]) * '+parseFloat(d[0])/100+")")}this.anchors[b].calculatedCoords=c;this.p.checkType(["image"])||Logger.warning(Logger.WARN_NOT_IMPLEMENTED_YET,"area",["coords",a.source.type])}}this.exec("end",$.proxy(function(){for(i in this.anchors)this.anchors[i].temporal&&this.stopArea(i)},this))}
Html5Player.prototype.stopArea=function(a){this.anchors[a].started?(this.anchors[a].started=!1,$(this.htmlPlayer).trigger("stop",[a])):$(this.htmlPlayer).trigger("presentation.onEnd",[a])};Html5Player.prototype.unload=function(){$(this.htmlPlayer).empty()};Html5Player.prototype.mediaNotFound=function(){this.loadTries--;0>=this.loadTries&&Logger.warning(Logger.WARN_MEDIA_NOT_FOUND,this.p.id,[this.p.source.url])};
...
LuaPlayer.prototype.load=function(a){var b=a.split("/");this.pathLua=b[0]+"/"+b[1]+"/";$.ajax({type:"GET",url:a,dataType:"text",async:!1,success:$.proxy(function(a){this.luajscode=lua_load("canvas = libCanvas.init();\n"+a)},this),error:function(){console.log("error to load file")}});this.bindlibs()};LuaPlayer.prototype.exec=function(a){console.log("exec lua: "+a)};
...
LuaPlayer.prototype.bindlibs=function(){lua_libs.libCanvas={variable:{id:this.id,p:this.p,canvas_objects:this.canvas_objects,pathLua:this.pathLua}};lua_libs.libCanvas.init=$.proxy(function(){var a=document.createElement("canvas");a.id="mycanvas_"+this.variable.id;try{a.width=this.variable.p.getProperty("width").split("px")[0],a.height=this.variable.p.getProperty("height").split("px")[0]}catch(b){a.width=500,a.height=500}$("#"+this.variable.p.id).append(a);a=a.getContext("2d");a=new libCanvas(a);console.log("init");
this.variable.canvas_objects[this.variable.id]=a;a=lua_newtable();a.str.id=this.variable.id;a.str["new"]=$.proxy(function(a,b,e){void 0===e?(objCanvas=this.variable.canvas_objects[a.str.id],newObject=objCanvas.newImage(this.variable.pathLua+b),this.variable.id+=1,this.variable.canvas_objects[this.variable.id]=newObject,a=$.extend(!0,{},a),a.str.id=this.variable.id):(objCanvas=this.variable.canvas_objects[a.str.id],newObject=objCanvas.newCanvas(b,e),this.variable.id+=1,a=$.extend(!0,{},a),a.str.id=
this.variable.id,this.variable.canvas_objects[this.variable.id]=newObject);return[a]},this);a.str.attrSize=$.proxy(function(a){return 0==this.variable.canvas_objects.length?(console.warn("This lua has no canvas."),[0,0]):this.variable.canvas_objects[a.str.id].attrSize()},this);a.str.attrColor=$.proxy(function(a,b,e,f,g){0==this.variable.canvas_objects.length?console.warn("This lua has no canvas."):this.variable.canvas_objects[a.str.id].attrColor(b,e,f,g)},this);a.str.drawLine=$.proxy(function(a,b,
e,f,g){0==this.variable.canvas_objects.length?console.warn("This lua has no canvas."):this.variable.canvas_objects[a.str.id].drawLine(b,e,f,g)},this);a.str.drawRect=$.proxy(function(a,b,e,f,g,k){0==this.variable.canvas_objects.length?console.warn("This lua has no canvas."):this.variable.canvas_objects[a.str.id].drawRect(b,e,f,g,k)},this);a.str.drawText=$.proxy(function(a,b,e,f){0==this.variable.canvas_objects.length?console.warn("This lua has no canvas."):this.variable.canvas_objects[a.str.id].drawText(b,
e,f)},this);a.str.measureText=$.proxy(function(a,b){return 0==this.variable.canvas_objects.length?(console.warn("This lua has no canvas."),[0,0]):[this.variable.canvas_objects[a.str.id].measureTextLua(b)]},this);a.str.attrText=$.proxy(function(a,b,e,f){0==this.variable.canvas_objects.length?console.warn("This lua has no canvas."):this.variable.canvas_objects[a.str.id].attrText(b,e,f)},this);a.str.attrCrop=$.proxy(function(a,b,e,f,g){0==this.variable.canvas_objects.length?console.warn("This lua has no canvas."):
this.variable.canvas_objects[a.str.id].attrCrop(b,e,f,g)},this);a.str.compose=$.proxy(function(a,b,e,f){0==this.variable.canvas_objects.length?console.warn("This lua has no canvas."):this.variable.canvas_objects[a.str.id].compose(b,e,this.variable.canvas_objects[f.str.id])},this);a.str.flush=$.proxy(function(a){0==this.variable.canvas_objects.length?console.warn("This lua has no canvas."):this.variable.canvas_objects[a.str.id].flush()},this);a.str.attrClip=$.proxy(function(a,b,e,f,g){0==this.variable.canvas_objects.length?
console.warn("This lua has no canvas."):this.variable.canvas_objects[a.str.id].attrClip(b,e,f,g)},this);return[a]},lua_libs.libCanvas);lua_libs.persistent={set:$.proxy(function(a,b,c){console.log(this);this.persitent.storeField(a,b,c)},this),get:$.proxy(function(a){return[this.persitent.recoverField(a)]},this)};lua_libs.event={post:$.proxy(function(a,b){this.events.post(a,b)},this),register:$.proxy(function(a){this.events.register(a)},this),unregister:$.proxy(function(a){this.events.unregister(a)},
this),timer:$.proxy(function(a,b){this.events.timer(a,b)},this),uptime:$.proxy(function(){return[this.events.uptime()]},this)};lua_libs.broker={init:$.proxy(function(a,b){this.broker.init(a,b)},this),post:$.proxy(function(a,b){this.broker.post(a,b)},this),register:$.proxy(function(a,b){this.broker.register(a,b)},this),unregister:$.proxy(function(a){this.broker.unregister(a)},this)}};
...
b[this.type].defaultPlayer);if(c)for(i in this.player=new c(this.playerSettings),this.playerName=c.name,c="load unload exec start stop pause resume abort seek seekAndPlay setProperty getDuration keyEventHandler".split(" "),c)this.player[c[i]]||(this.player.__playerName||(this.player.__playerName=this.playerName),this.player[c[i]]=function(){Logger.error(Logger.ERR_MEDIAPLAYER_METHOD_NOTFOUND,this.__playerName,[arguments.callee.name])});else Logger.error(Logger.ERR_MEDIAPLAYER_NOPLAYER,this.type,[]),
this.player={};$(this.htmlPlayerBkg).css("display","none");this.load(a.src);this.data=$(this.htmlPlayer).data();this.player.exec&&this.player.exec("end",$.proxy(function(){if(this.playingArea){$(this.htmlPlayer).trigger("presentation.onEnd");this.playingArea=void 0}else this.stop()},this));$(this.htmlPlayer).data("property",[]);if(a.descriptor)for(i in a.descriptor.explicitDur&&this.setProperty("explicitDur",a.descriptor.explicitDur),a.descriptor.region&&(this.setProperty("zIndex",a.descriptor.region.zIndex),
this.setProperty("width",a.descriptor.region.width),this.setProperty("height",a.descriptor.region.height),this.setProperty("bottom",a.descriptor.region.bottom),this.setProperty("top",a.descriptor.region.top),this.setProperty("right",a.descriptor.region.right),this.setProperty("left",a.descriptor.region.left),this.setProperty("title",a.descriptor.region.title)),a.descriptor.descriptorParam)this.setProperty(a.descriptor.descriptorParam[i].name,a.descriptor.descriptorParam[i].value);c=[];for(i in a.property)if(a.property[i].value)switch(a.property[i].name){case "bounds":case "top":case "left":case "bottom":case "right":case "height":case "width":c.push({name:a.property[i].name,
value:a.property[i].value});break;default:this.setProperty(a.property[i].name,a.property[i].value)}if(0<c.length){a={left:0,top:0,width:parseInt($("#"+this.presentation.playerDiv).css("width").split("px")[0]),height:parseInt($("#"+this.presentation.playerDiv).css("height").split("px")[0])};b={left:null,right:null,top:null,bottom:null,width:null,height:null};this.node.descriptor&&this.node.descriptor.region&&(a.left=this.node.descriptor.region._parent.left?parseInt(this.node.descriptor.region._parent.left.split("px")[0]):
...
(this.node.descriptor.region.left.split("px")[0]-a.left).toString(),b.top=(this.node.descriptor.region.top.split("px")[0]-a.top).toString(),b.width=this.node.descriptor.region.width,b.height=this.node.descriptor.region.height);for(i in c){var d=c[i].name,e=c[i].value;switch(d){case "bounds":e=e.split(",");b.left=e[0];b.top=e[1];b.width=e[2];b.height=e[3];break;case "top":case "left":case "bottom":case "right":case "height":case "width":b[d]=e}}WebNclPlayer.prototype.fixRegionBounds(b,a);for(d in b)(e=
b[d])&&null!=e&&this.setProperty(d,e)}this.player.exec("end",$.proxy(function(){if(this.playingArea){$(this.htmlPlayer).trigger("presentation.onEnd");this.playingArea=void 0}else this.stop()},this));this.bindEvents();$(this.htmlPlayer).data("player",this)}};MediaPlayer.prototype.load=function(a){a=this.presentation.path+a;this.player.unload(a);this.player.load(a)};
...
parseInt($(this.htmlPlayer).css("height").split("px")[0])+"px",e.top=$(this.htmlPlayer).css("top"))}if("out"==b)for(i in d)g=d[i],d[i]=e[i],e[i]=g;break;case "irisWipe":a.subtype=a.subtype||"rectangle";f=this.htmlPlayerBkg;if("rectangle"==a.subtype){var g=parseInt($(this.htmlPlayer).css("left").split("px")[0]),k=parseInt($(this.htmlPlayer).css("top").split("px")[0]),h=parseInt($(this.htmlPlayer).css("width").split("px")[0]),n=parseInt($(this.htmlPlayer).css("height").split("px")[0]);d.width="0px";
d.height="0px";d.left=g+h/2+"px";d.top=k+n/2+"px";e.width=h+"px";e.height=n+"px";e.left=g+"px";e.top=k+"px";if("out"==b)for(i in d)g=d[i],d[i]=e[i],e[i]=g}else Logger.warning(Logger.WARN_NOT_IMPLEMENTED_YET,"transition",[a.type,a.subtype]);break;case "snakeWipe":case "clockWipe":Logger.warning(Logger.WARN_NOT_IMPLEMENTED_YET,"transition",[a.type])}$(f).css(d);$(f).animate(e,c);$(f).dequeue()};
