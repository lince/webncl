...
$("#areaSpace1").hide();$("#areaPowerEject").hide();$("#areaSpace2").hide();$("#areaNumbers").hide();$("#areaSpace3").hide();$("#areaVolumeChannel").hide();$("#areaSpace4").hide();$("#areaInteractive").hide();$("#areaSpace5").hide();$("#areaColorful").hide();$("#areaSpace6").hide();$("#areaMultimedia").hide();$("#areaSpace7").hide();$("#areaLogo").hide();$("#areaSpace8").hide();this.setAreas();a="Eject Power Key_1 Key_2 Key_3 Key_4 Key_5 Key_6 Key_7 Key_8 Key_9 Key_Hash Key_0 Key_Star Volume_Up Channel_Up Volume_Down Channel_Down Menu Mute Help Info Guide Exit Back Enter Cursor_Up Cursor_Down Cursor_Left Cursor_Right Red Green Yellow Blue Rewind Play Forward Pause Record Stop".split(" ");
for(button in a)eval("$('#Button' + imgButtons[button]).click($.proxy(function(){this.functionButton('"+a[button].toUpperCase()+"');},this));")}RemoteControl.prototype.setAreas=function(){var a="PowerEject Numbers VolumeChannel Interactive Colorful Multimedia".split(" ");for(area in a)if(this["p_area"+a[area]])this["setArea"+a[area]]()};RemoteControl.prototype.functionButton=function(a){this.player.keyPress(a)};var areas="PowerEject Numbers VolumeChannel Interactive Colorful Multimedia".split(" ");
...
"');},this));"));if(this.anchors[b].spatial&&!this.anchors[b]._ignore){var c=this.anchors[b].coords.split(",");for(i in c){var d=c[i].split("%");1<d.length&&(c[i]='Math.round(parseInt($(this.htmlPlayer).css("'+(i%2?"height":"width")+'").split("px")[0]) * '+parseFloat(d[0])/100+")")}this.anchors[b].calculatedCoords=c;this.p.checkType(["image"])||Logger.warning(Logger.WARN_NOT_IMPLEMENTED_YET,"area",["coords",a.source.type])}}this.exec("end",$.proxy(function(){for(i in this.anchors)this.anchors[i].temporal&&
...
Html5Player.prototype.load=function(a){this.p.source.url=a;this.loadTries=1;var b=this.p.source.type.split("/")[0];switch(b){case "video":case "audio":var c=a.substr(0,a.lastIndexOf(".")),d=a.substr(a.lastIndexOf(".")+1),a="video"==b?["webm","mp4","ogg"]:["mp3","ogg"];a.sort(function(a,b){return b==d});this.loadTries=a.length;for(i in a)$(this.htmlPlayer).append('<source type="'+b+"/"+a[i]+'" src="'+c+"."+a[i]+'"></source>');break;case "image":$(this.htmlPlayer).attr("src",a);break;case "text":this.p.checkType(["text/plain",
"text/html"])&&$.ajax({type:"GET",url:a,dataType:"text",success:$.proxy(function(a){this.textData=a;this.loadTextData()},this),error:$.proxy(this.mediaNotFound,this)})}};Html5Player.prototype.exec=function(a,b){if(this.popcornPlayer)if("begin"==a)$(this.htmlPlayer).on("play",b);else"end"==a?(this.endCallbacks.push(b),$(this.htmlPlayer).on("ended",b)):this.popcornPlayer.cue(a,b)};
...
this.p.onChangeProperty.propertyMap.width=Player.propertyAction.IGNORE;this.p.onChangeProperty.propertyMap.height=Player.propertyAction.IGNORE;this.p.onChangeProperty.propertyMap.left=Player.propertyAction.IGNORE;this.p.onChangeProperty.propertyMap.top=Player.propertyAction.IGNORE;a.enableKeyEvents();console.log(a.id)}LuaPlayer.prototype.unload=function(){$(this.htmlPlayer).empty()};
...
LuaPlayer.prototype.bindlibs=function(){lua_libs.libCanvas={variable:{id:this.id,p:this.p,canvas_objects:this.canvas_objects}};lua_libs.libCanvas.init=$.proxy(function(){var a=document.createElement("canvas");a.id="mycanvas_"+this.variable.id;a.width=this.variable.p.getProperty("width").split("px")[0];a.height=this.variable.p.getProperty("height").split("px")[0];$("#"+this.variable.p.id).append(a);a=a.getContext("2d");a=new libCanvas(a);console.log("init");this.variable.canvas_objects[this.variable.id]=
a;a=lua_newtable();a.str.id=this.variable.id;a.str["new"]=$.proxy(function(a,c,d){void 0===d?(objCanvas=this.variable.canvas_objects[a.str.id],newObject=objCanvas.newImage(c),this.variable.id+=1,this.variable.canvas_objects[this.variable.id]=newObject,a=$.extend(!0,{},a),a.str.id=this.variable.id):(objCanvas=this.variable.canvas_objects[a.str.id],newObject=objCanvas.newCanvas(c,d),this.variable.id+=1,a=$.extend(!0,{},a),a.str.id=this.variable.id,this.variable.canvas_objects[this.variable.id]=newObject);
return[a]},this);a.str.attrSize=$.proxy(function(a){return this.variable.canvas_objects[a.str.id].attrSize()},this);a.str.attrColor=$.proxy(function(a,c,d,e,f){this.variable.canvas_objects[a.str.id].attrColor(c,d,e,f)},this);a.str.drawLine=$.proxy(function(a,c,d,e,f){this.variable.canvas_objects[a.str.id].drawLine(c,d,e,f)},this);a.str.drawRect=$.proxy(function(a,c,d,e,f,g){this.variable.canvas_objects[a.str.id].drawRect(c,d,e,f,g)},this);a.str.drawText=$.proxy(function(a,c,d,e){this.variable.canvas_objects[a.str.id].drawText(c,
d,e)},this);a.str.measureText=$.proxy(function(a,c){return this.variable.canvas_objects[a.str.id].measureTextLua(c)},this);a.str.attrText=$.proxy(function(a){this.variable.canvas_objects[a.str.id].attrText(face,size,style)},this);a.str.attrCrop=$.proxy(function(a,c,d,e,f){this.variable.canvas_objects[a.str.id].attrCrop(c,d,e,f)},this);a.str.compose=$.proxy(function(a,c,d,e){this.variable.canvas_objects[a.str.id].compose(c,d,this.variable.canvas_objects[e.str.id])},this);a.str.flush=$.proxy(function(a){this.variable.canvas_objects[a.str.id].flush()},
this);a.str.attrClip=$.proxy(function(a,c,d,e,f){this.variable.canvas_objects[a.str.id].attrClip(c,d,e,f)},this);return[a]},lua_libs.libCanvas);persist=this.persitent;lua_libs.persistent={set:function(a,b,c){persist.storeField(a,b,c)},get:function(a){return[persist.recoverField(a)]}};events=this.events;lua_libs.event={post:function(a,b){events.post(a,b)},register:function(a){events.register(a)},unregister:function(a){events.unregister(a)},timer:function(a,b){events.timer(a,b)},uptime:function(){return[events.uptime()]}};
broker=this.broker;lua_libs.broker={init:function(a,b){broker.init(a,b)},post:function(a,b){broker.post(a,b)},register:function(a,b){broker.register(a,b)},unregister:function(a){broker.unregister(a)}}};
...
(this.player.__playerName=this.playerName),this.player[b[i]]=function(){Logger.error(Logger.ERR_MEDIAPLAYER_METHOD_NOTFOUND,this.__playerName,[arguments.callee.name])});else Logger.error(Logger.ERR_MEDIAPLAYER_NOPLAYER,this.type,["no defaultPlayer or extension player"]),this.player={};else Logger.error(Logger.ERR_MEDIAPLAYER_NOPLAYER,this.type),this.player={};$(this.htmlPlayerBkg).css("display","none");this.load(a.src);this.data=$(this.htmlPlayer).data();this.player.exec&&this.player.exec("end",$.proxy(function(){if(this.playingArea){$(this.htmlPlayer).trigger("presentation.onEnd");
this.playingArea=void 0}else this.stop()},this));$(this.htmlPlayer).data("property",[]);if(a.descriptor)for(i in a.descriptor.explicitDur&&this.setProperty("explicitDur",a.descriptor.explicitDur),a.descriptor.region&&(this.setProperty("zIndex",a.descriptor.region.zIndex),this.setProperty("width",a.descriptor.region.width),this.setProperty("height",a.descriptor.region.height),this.setProperty("bottom",a.descriptor.region.bottom),this.setProperty("top",a.descriptor.region.top),this.setProperty("right",
a.descriptor.region.right),this.setProperty("left",a.descriptor.region.left),this.setProperty("title",a.descriptor.region.title)),a.descriptor.descriptorParam)this.setProperty(a.descriptor.descriptorParam[i].name,a.descriptor.descriptorParam[i].value);b=[];for(i in a.property)if(a.property[i].value)switch(a.property[i].name){case "bounds":case "top":case "left":case "bottom":case "right":case "height":case "width":b.push({name:a.property[i].name,value:a.property[i].value});break;default:this.setProperty(a.property[i].name,
a.property[i].value)}if(0<b.length){var a={left:0,top:0,width:parseInt($("#"+this.presentation.playerDiv).css("width").split("px")[0]),height:parseInt($("#"+this.presentation.playerDiv).css("height").split("px")[0])},c={left:null,right:null,top:null,bottom:null,width:null,height:null};this.node.descriptor&&this.node.descriptor.region&&(a.left=this.node.descriptor.region._parent.left?parseInt(this.node.descriptor.region._parent.left.split("px")[0]):0,a.top=this.node.descriptor.region._parent.top?parseInt(this.node.descriptor.region._parent.top.split("px")[0]):
0,a.width=this.node.descriptor.region._parent.width?parseInt(this.node.descriptor.region._parent.width.split("px")[0]):parseInt($("#"+this.presentation.playerDiv).css("width").split("px")[0]),a.height=this.node.descriptor.region._parent.height?parseInt(this.node.descriptor.region._parent.height.split("px")[0]):parseInt($("#"+this.presentation.playerDiv).css("height").split("px")[0]),c.left=(this.node.descriptor.region.left.split("px")[0]-a.left).toString(),c.top=(this.node.descriptor.region.top.split("px")[0]-
a.top).toString(),c.width=this.node.descriptor.region.width,c.height=this.node.descriptor.region.height);for(i in b){var d=b[i].name,e=b[i].value;switch(d){case "bounds":e=e.split(",");c.left=e[0];c.top=e[1];c.width=e[2];c.height=e[3];break;case "top":case "left":case "bottom":case "right":case "height":case "width":c[d]=e}}WebNclPlayer.prototype.fixRegionBounds(c,a);for(d in c)(e=c[d])&&null!=e&&this.setProperty(d,e)}this.player.exec("end",$.proxy(function(){if(this.playingArea){$(this.htmlPlayer).trigger("presentation.onEnd");
this.playingArea=void 0}else this.stop()},this));this.bindEvents();$(this.htmlPlayer).data("player",this)}};MediaPlayer.prototype.load=function(a){a=this.presentation.path+a;this.player.unload(a);this.player.load(a)};
