...
MediaPlayer.prototype.start=function(a){this.isStopped&&(this.presentation.focusManager.enableKeys(this.htmlPlayer),this.presentation.focusManager.addMedia(this.node.descriptor.focusIndex,this.htmlPlayer),this.isPlaying=!0,this.isStopped=!1,this.show(),this.checkType(["video","audio","image","text"])&&(this.parentContext.syncPlayer(this),this.popcornPlayer.play(),a&&"area"==this.area[a]._type?this.area[this.area[a].id].begin&&(this.area[this.area[a].id].started=!0,this.seek(this.area[this.area[a].id].beginTime),
$(this.htmlPlayer).one("seeked",$.proxy(function(){this.parentContext.notify(this)},this))):this.parentContext.notify(this)),$(this.htmlPlayer).trigger("presentation.onBegin",[a]))};
