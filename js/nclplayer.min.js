...
a+"] "+this.MESSAGES[a]+"\n"+b+": "+c.join(","))}};function NclPlayer(a,b){this.presentation={TIME_LIMIT:1E3,playerId:++NclPlayer.playerCount,playerDiv:b,getDivId:function(a,b){return"ncl"+this.playerId+(b||"")+"_"+a}};this.presentation.bodyDiv="body"+this.presentation.playerId;this.presentation.settingsDiv="settings"+this.presentation.playerId;this.presentation.contextsDiv="contexts"+this.presentation.playerId;this.presentation.keyEvents={};$.ajax({type:"GET",url:a,dataType:"xml",success:$.proxy(function(a){this.execute(a)},this)})}
NclPlayer.prototype.execute=function(a){var b=new Date;this.presentation.parser=new Parser;this.presentation.ncl=this.presentation.parser.parse(a);console.log(new Date-b);this.presentation.focusManager=new FocusManager(this.presentation);this.presentation.systemSettings=new SystemSettings(this.presentation);for(rb in this.presentation.ncl.head.regionBase)for(i in this.presentation.ncl.head.regionBase[rb].region)a={left:0,top:0,width:parseInt($("#"+this.presentation.playerDiv).css("width").split("px")[0]),
...
SystemSettings.prototype.setPropertyValue=function(a,b){if(a in this.settings)this.settings[a]=b;else if(a in this.setMap)this.setMap[a](b);else return!1;return!0};SystemSettings.prototype.getPropertyValue=function(a){return a in this.settings?this.settings[a]:a in this.setMap?this.getMap[a]():""};function Parser(){this.referenceMap={map:[],addSource:function(a,b,c){this.map[a[b]]||(this.map[a[b]]={sources:[]});this.map[a[b]].sources.push({obj:a,attr:b,type:c})},addTarget:function(a,b,c,d){var e=!1;if(this.map[b])if(this.map[b].target)if("property"==d||"connectorParam"==d){if(this.map[b].target.parents[c])return!1}else return!1;else e=!0;else this.map[b]={sources:[]},e=!0;e&&(this.map[b].target={obj:a,type:d,parents:[]});return this.map[b].target.parents[c]=!0},pop:function(a){return this.map[a]?
this.map[a].target:!1},createReferences:function(){for(id in this.map)for(i in this.map[id].sources){var a=this.map[id].sources[i];this.map[id].target&&this.map[id].target.obj&&-1!=$.inArray(this.map[id].target.type,a.type)?a.obj[a.attr]=this.map[id].target.obj:Debugger.error(Debugger.ERR_INVALID_ID_REFERENCE,a.type,[a.attr,a.obj[a.attr],a.type])}}};this.uniqueTable=[]}Parser.prototype.parse=function(a){this.ncl=this.createNode(a);this.referenceMap.createReferences();return this.ncl};
Parser.prototype.createNode=function(a,b,c,d){b=b||"ncl";a=$(a).find("ncl"==b?"ncl":"> "+b);if(0==a.length)return!1;var e=[],f=Parser.nclStructureMap[b].attrs;f.required||(f.required=[]);f.optional||(f.optional=[]);f.oneOf||(f.oneOf=[]);var g=[].concat(f.required,f.optional,f.oneOf),f=Parser.nclStructureMap[b].content;f.optional||(f.optional=[]);f.one||(f.one=[]);f.plus||(f.plus=[]);f.plusOneOf||(f.plusOneOf=[]);f.star||(f.star=[]);f.custom||(f.custom=[]);var j=[].concat(f.optional,f.one,f.plus,f.plusOneOf,
f.star,f.custom),m=this;$(a).each(function(){var a={_type:b};for(i in g)a[g[i]]=$(this).attr(g[i]);var f=d+">"+b;a.id&&(f+="#"+a.id);m.parseAttributes(this,a);m.parseContent(this,a);for(i in j)a[j[i]]=m.createNode(this,j[i],a,f);e.push(a);if("ncl"==b)m.parseNCL(a,b,c,d);else m["parse"+b[0].toUpperCase()+b.slice(1,b.length)](a,b,c,d)});return-1==$.inArray(b,Parser.isNotArray)?e:e[0]};
...
ContextPlayer.prototype.startPort=function(a){switch(a.component._type){case "context":this.context[a.component.id]?a["interface"]?(this.context[a.component.id].create(),this.context[a.component.id].port[a["interface"].id]?this.context[a.component.id].start(a["interface"].id):Debugger.error(Debugger.ERR_INVALID_CONTEXT_REFERENCE,"port",[a.id,"interface",a["interface"].id])):this.context[a.component.id].start():Debugger.error(Debugger.ERR_INVALID_CONTEXT_REFERENCE,"port",[a.id,"component",a.component.id]);
break;case "media":a["interface"]?this.media[a.component.id].start(a["interface"].id):this.media[a.component.id].start()}};ContextPlayer.prototype.stop=function(a){if(!this.isStopped){this.isPlaying=!1;this.isStopped=!0;for(i in this.context)this.context[i].stop();for(i in this.media)this.media[i].stop();this.pausedMediaList=[];this.pausedContextList=[];$(this.htmlPlayer).trigger("presentation.onEnd",[a])}};
...
ContextPlayer.prototype.bindLinks=function(){var a=this.node.port,b=this.node.link,c=this.handlers,d;for(d in a){var e=a[d],f=e["interface"]||void 0;if("media"==e.component._type&&f)console.warn("Eventos de atribuicao para propriedades referenciadas por portas nao foi implementado");else{var f="#"+this.presentation.getDivId(e.component.id),e=e.id,g;for(g in Parser.eventType){var j=Parser.eventType[g];if("attribution"!=j){var m=[j,g].join(".");$(f).on(m,{contextElement:this.htmlPlayer,interfaceId:e,
eventName:m},function(a){var b=$.Event(a.data.eventName);b.which=a.which;$(a.data.contextElement).trigger(b,[a.data.interfaceId])})}}}}for(d in b){var a=b[d],h=a.xconnector,f={},e={},p=0,m="par",l=h.simpleCondition,q={},r={},k=[];if(h.connectorParam)for(d in h.connectorParam)currentConnectorParam=h.connectorParam[d],q["$"+currentConnectorParam.name]="";if(h.compoundCondition)for(d in l=h.compoundCondition[0].simpleCondition,1<l.length&&(p=Listener.listenerType[h.compoundCondition[0].operator.toUpperCase()]),
h.compoundCondition[0].assessmentStatement)for(g in j=h.compoundCondition[0].assessmentStatement[d],j.attributeAssessment){var n=j.attributeAssessment[g];"attribution"==n.eventType&&(!n.attributeType||"nodeProperty"==n.attributeType?r[n.role]={bindComponent:"",bindInterface:"",operator:j.comparator,value:j.valueAssessment.value,variable:ContextPlayer.isVariable(j.valueAssessment.value)}:console.warn("Eventos de tipos diferentes de attribution nao foram implementados para elementos do tipo AttributeAssessment"))}for(g in r)k.push(r[g]);
for(d in l){var j=l[d],n=!1,o="",s=!1;"onSelection"==j.role&&j.key&&(ContextPlayer.isVariable(j.key)&&(n=!0),o=j.key,s=!0);e[j.role]={bindComponent:"",bindInterface:null,eventType:Parser.eventType[j.role],flag:!1,keyUseVariable:n,keyDefaultValue:o,useKey:s}}l=h.simpleAction;h.compoundAction&&(m=h.compoundAction[0].operator.toLowerCase(),l=h.compoundAction[0].simpleAction);for(d in l)h=l[d],"set"==h.role?(j=!1,n="",h.value&&(ContextPlayer.isVariable(h.value)&&(j=!0),n=h.value),f[h.role]={binds:[],
qualifier:l[d].qualifier,actionUseVariable:j,actionDefaultValue:n}):f[h.role]={binds:[],qualifier:l[d].qualifier};p=new Listener(p,m,f,e,k);if(a.linkParam)for(d in a.linkParam)currentLinkParam=a.linkParam[d],q[currentLinkParam.name]=currentLinkParam.value;for(d in a.bind)k=a.bind[d],n=k.component.id,m="#"+this.presentation.getDivId(n),l=k["interface"],f[k.role]?(j=null,h=f[k.role],"set"==k.role&&(h.actionUseVariable?(k=ContextPlayer.createLocalParamMap(k.bindParam),j=k[h.actionDefaultValue]?k[h.actionDefaultValue]:
q[h.actionDefaultValue]):j=h.actionDefaultValue),h.binds.push({bindComponent:m,bindInterface:l,value:j})):e[k.role]?(h=e[k.role],h.bindComponent=m,h.bindInterface=l,l=m,j=Parser.eventType[k.role],m=k.role,j=[j,m].join("."),n=[n,j].join("."),o=j+".triggerArray",h.useKey&&(k=ContextPlayer.createLocalParamMap(k.bindParam),h.keyDefaultValue=k[h.keyDefaultValue]?k[h.keyDefaultValue]:q[h.keyDefaultValue],h.keyDefaultValue in Keys&&(h.keyDefaultValue=Keys[h.keyDefaultValue]),this.presentation.keyEvents[l]=
!1),c[n]||(c[n]=!0,$(l).data(o)||$(l).data(o,[]),$(l).data(j)||$(l).data(j,[]),$(l).on(j,{componentDivId:l,bindName:j,eventName:m,triggerArrayName:o},function(a,b){var c=$(a.data.componentDivId).data(a.data.bindName),d;for(d in c){var e=c[d],f=e.flagMap[a.data.eventName];b==f.bindInterface&&(!a.which&&!f.usekey?e.notifyEvent(a.data.eventName):a.which&&f.keyDefaultValue==a.which&&e.notifyEvent(a.data.eventName))}ContextPlayer.executeTriggerArray($(a.data.componentDivId).data(a.data.triggerArrayName))})),
p.triggerBuffer=$(l).data(o),$(l).data(j).push(p)):(currentAssessment=r[k.role],currentAssessment.bindComponent=m,currentAssessment.bindInterface=l,currentAssessment.variable&&(k=ContextPlayer.createLocalParamMap(k.bindParam),currentAssessment.value=k[currentAssessment.value]?k[currentAssessment.value]:q[currentAssessment.value]))}};MediaPlayer.prototype=new Player;function MediaPlayer(a,b){this.htmlPlayerBkg=this.htmlPlayer=this.type=this.region="";this.popcornPlayer={};this.area=[];this.transIn=[];this.transOut=[];this.isPlaying=!1;this.isVisible=this.isStopped=!0;this.isFocused=!1;this.opacity=1;this.node=a;this.presentation=b.presentation;this.parentContext=b;this.create(a)}MediaPlayer.prototype.checkType=function(a){var b=this.type.split("/"),c;for(c in a){var d=a[c].split("/");if(d[0]==b[0]&&(!d[1]||b[1]&&d[1]==b[1]))return!0}return!1};
MediaPlayer.prototype.create=function(a){a.descriptor&&(this.transIn=a.descriptor.transIn,this.transOut=a.descriptor.transOut);for(i in a.area)this.area[a.area[i].id]=a.area[i],a.area[i].begin&&(this.area[a.area[i].id].beginTime=parseInt(a.area[i].begin.split("s")[0])),a.area[i].end&&(this.area[a.area[i].id].endTime=parseInt(a.area[i].end.split("s")[0]),this.area[a.area[i].id].started=!1);if(a.type)this.type=a.type;else{var b=a.src.split(".");this.type=Parser.mediaTypes[b[b.length-1]]}this.htmlPlayer=
"#"+this.presentation.getDivId(a.id);this.htmlPlayerBkg="#"+this.presentation.getDivId(a.id,"bkg");this.region=this.checkType(["application"])?"#"+this.presentation.settingsDiv:"#"+this.presentation.playerDiv;$(this.region).append("<div class='playerBkg' id='"+this.presentation.getDivId(a.id,"bkg")+"'></div>");switch(this.type.split("/")[0]){case "video":$(this.htmlPlayerBkg).append("<video class='player' poster='images/loading.gif' id='"+this.presentation.getDivId(a.id)+"'></video>");break;case "audio":$(this.htmlPlayerBkg).append("<audio class='player' id='"+
...
a.property[i].value);this.bindEvents();$(this.htmlPlayer).data("player",this)};
...
parseInt($(this.htmlPlayer).css("height").split("px")[0])+"px",e.top=$(this.htmlPlayer).css("top"))}if("out"==b)for(i in d)g=d[i],d[i]=e[i],e[i]=g;break;case "irisWipe":a.subtype=a.subtype||"rectangle";f=this.htmlPlayerBkg;if("rectangle"==a.subtype){var g=parseInt($(this.htmlPlayer).css("left").split("px")[0]),j=parseInt($(this.htmlPlayer).css("top").split("px")[0]),m=parseInt($(this.htmlPlayer).css("width").split("px")[0]),h=parseInt($(this.htmlPlayer).css("height").split("px")[0]);d.width="0px";
d.height="0px";d.left=g+m/2+"px";d.top=j+h/2+"px";e.width=m+"px";e.height=h+"px";e.left=g+"px";e.top=j+"px";if("out"==b)for(i in d)g=d[i],d[i]=e[i],e[i]=g}else Debugger.warning(Debugger.WARN_NOT_IMPLEMENTED_YET,"transition",[a.type,a.subtype]);break;case "snakeWipe":case "clockWipe":Debugger.warning(Debugger.WARN_NOT_IMPLEMENTED_YET,"transition",[a.type])}$(f).css(d);$(f).animate(e,c);$(f).dequeue()};
...
MediaPlayer.prototype.start=function(a){this.isStopped&&(this.presentation.focusManager.enableKeys(this.htmlPlayer),this.presentation.focusManager.addMedia(this.node.descriptor.focusIndex,this.htmlPlayer),this.isPlaying=!0,this.isStopped=!1,this.show(),this.checkType(["audio","video"])&&(this.parentContext.syncPlayer(this),a&&"area"==this.area[a]._type?this.area[this.area[a].id].begin&&(this.area[this.area[a].id].started=!0,this.seek(this.area[this.area[a].id].beginTime),$(this.htmlPlayer).one("seeked",
$.proxy(function(){this.parentContext.notify(this)},this))):this.parentContext.notify(this)),$(this.htmlPlayer).trigger("presentation.onBegin",[a]))};
MediaPlayer.prototype.stop=function(a){this.isStopped||(this.presentation.focusManager.disableKeys(this.htmlPlayer),this.presentation.focusManager.removeMedia(this.node.descriptor.focusIndex,this.htmlPlayer),this.isPlaying=!1,this.isStopped=!0,this.hide(),this.checkType(["video","audio","image","text"])&&this.popcornPlayer.pause(),$(this.htmlPlayer).trigger("presentation.onEnd",[a]))};
...
MediaPlayer.prototype.abort=function(a){this.isStopped||(this.presentation.focusManager.removeMedia(this.node.descriptor.focusIndex,this.htmlPlayer),this.isPlaying=!1,this.isStopped=!0,this.hide(),this.checkType(["video","audio","image","text"])&&this.popcornPlayer.pause(),$(this.htmlPlayer).trigger("presentation.onAbort",[a]))};
