...
a+"] "+this.MESSAGES[a]+"\n"+b+": "+c.join(","))}};function NclPlayer(a,b){this.presentation={TIME_LIMIT:1E3,playerId:++NclPlayer.playerCount,playerDiv:b,getDivId:function(a,b){return"ncl"+this.playerId+(b||"")+"_"+(this.parser.referenceMap.instReuse[a]||a)}};this.presentation.bodyDiv="body"+this.presentation.playerId;this.presentation.settingsDiv="settings"+this.presentation.playerId;this.presentation.contextsDiv="contexts"+this.presentation.playerId;this.presentation.keyEvents={};$.ajax({type:"GET",url:a,dataType:"xml",success:$.proxy(function(a){this.execute(a)},
this)})}
NclPlayer.prototype.execute=function(a){var b=new Date;this.presentation.parser=new Parser;this.presentation.ncl=this.presentation.parser.parse(a);console.log(new Date-b);this.presentation.focusManager=new FocusManager(this.presentation);this.presentation.systemSettings=new SystemSettings(this.presentation);for(rb in this.presentation.ncl.head.regionBase)for(i in this.presentation.ncl.head.regionBase[rb].region)a={left:0,top:0,width:parseInt($("#"+this.presentation.playerDiv).css("width").split("px")[0]),height:parseInt($("#"+
...
"#"+this.presentation.getDivId(a.id);this.htmlPlayerBkg="#"+this.presentation.getDivId(a.id,"bkg");this.region=this.checkType(["application"])?"#"+this.presentation.settingsDiv:"#"+this.presentation.playerDiv;if(!(a.refer&&"new"!=a.instance)){$(this.region).append("<div class='playerBkg' id='"+this.presentation.getDivId(a.id,"bkg")+"'></div>");switch(this.type.split("/")[0]){case "video":$(this.htmlPlayerBkg).append("<video class='player' poster='images/loading.gif' id='"+this.presentation.getDivId(a.id)+
...
MediaPlayer.prototype.start=function(a){this.isStopped&&(this.presentation.focusManager.enableKeys(this.htmlPlayer),this.presentation.focusManager.addMedia(this.node.descriptor.focusIndex,this.htmlPlayer),this.isPlaying=!0,this.isStopped=!1,this.show(),this.checkType(["audio","video"])&&(this.popcornPlayer.play(),a&&"area"==this.area[a]._type&&this.area[this.area[a].id].begin&&(this.area[this.area[a].id].started=!0,this.seek(this.area[this.area[a].id].beginTime),$(this.htmlPlayer).one("seeked",$.proxy(function(){},
this)))),$(this.htmlPlayer).trigger("presentation.onBegin",[a]))};MediaPlayer.prototype.stop=function(a){this.isStopped||(this.presentation.focusManager.disableKeys(this.htmlPlayer),this.presentation.focusManager.removeMedia(this.node.descriptor.focusIndex,this.htmlPlayer),this.isPlaying=!1,this.isStopped=!0,this.hide(),this.checkType(["video","audio","image","text"])&&this.popcornPlayer.pause(),$(this.htmlPlayer).trigger("presentation.onEnd",[a]))};
