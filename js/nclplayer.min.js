...
height:parseInt($("#"+this.presentation.playerDiv).css("height").split("px")[0])},this.fixRegionBounds(this.presentation.ncl.head.regionBase[rb].region[i],a);$("#"+this.presentation.playerDiv).append("<div id='"+this.presentation.settingsDiv+"'></div>");$("#"+this.presentation.playerDiv).append("<div id='"+this.presentation.contextsDiv+"'></div>");(new ContextPlayer(this.presentation.ncl.body,this.presentation)).start()};
...
ContextPlayer.prototype.bindLinks=function(){var a=this.node.port,b=this.node.link,c=this.handlers,d;for(d in a){var e=a[d],f=e["interface"]||void 0;if("media"==e.component._type&&f)console.warn("Eventos de atribuicao para propriedades referenciadas por portas nao foi implementado");else{var f="#"+this.presentation.getDivId(e.component.id),e=e.id,g;for(g in Parser.eventType){var j=Parser.eventType[g];if("attribution"!=j){var h=[j,g].join(".");$(f).on(h,{contextElement:this.htmlPlayer,interfaceId:e,
eventName:h},function(a){$(a.data.contextElement).trigger(a.data.eventName,[a.data.interfaceId])})}}}}for(d in b){var a=b[d],h=a.xconnector,f={},e={},n=0,j="par",k=h.simpleCondition,p={},q={},m=[];if(h.connectorParam)for(d in h.connectorParam)currentConnectorParam=h.connectorParam[d],p["$"+currentConnectorParam.name]="";if(h.compoundCondition)for(d in k=h.compoundCondition[0].simpleCondition,1<k.length&&(n=Listener.listenerType[h.compoundCondition[0].operator.toUpperCase()]),h.compoundCondition[0].assessmentStatement){var l=
h.compoundCondition[0].assessmentStatement[d];for(g in l.attributeAssessment){var o=l.attributeAssessment[g];"attribution"==o.eventType&&(!o.attributeType||"nodeProperty"==o.attributeType?q[o.role]={bindComponent:"",bindInterface:"",operator:l.comparator,value:l.valueAssessment.value,variable:ContextPlayer.isVariable(l.valueAssessment.value)}:console.warn("Eventos de tipos diferentes de attribution nao foram implementados para elementos do tipo AttributeAssessment"))}}for(g in q)m.push(q[g]);for(d in k)e[k[d].role]=
{bindComponent:"",bindInterface:null,eventType:Parser.eventType[k[d].role],flag:!1};k=h.simpleAction;h.compoundAction&&(j=h.compoundAction[0].operator.toLowerCase(),k=h.compoundAction[0].simpleAction);for(d in k)h=k[d],"set"==h.role?(l=!1,o="",h.value&&(ContextPlayer.isVariable(h.value)&&(l=!0),o=h.value),f[h.role]={binds:[],qualifier:k[d].qualifier,actionUseVariable:l,actionDefaultValue:o}):f[h.role]={binds:[],qualifier:k[d].qualifier};n=new Listener(n,j,f,e,m);if(a.linkParam)for(d in a.linkParam)currentLinkParam=
a.linkParam[d],p[currentLinkParam.name]=currentLinkParam.value;for(d in a.bind)m=a.bind[d],k=m.component.id,j="#"+this.presentation.getDivId(k),l=m["interface"],f[m.role]?(k=null,h=f[m.role],"set"==m.role&&(h.actionUseVariable?(localParamMap=ContextPlayer.createLocalParamMap(m.bindParam),k=localParamMap[h.actionDefaultValue]?localParamMap[h.actionDefaultValue]:p[h.actionDefaultValue]):k=h.actionDefaultValue),h.binds.push({bindComponent:j,bindInterface:l,value:k})):e[m.role]?(e[m.role].bindComponent=
j,e[m.role].bindInterface=l,l=j,j=Parser.eventType[m.role],h=m.role,j=[j,h].join("."),m=[k,j].join("."),k=j+".triggerArray",c[m]||(c[m]=!0,$(l).data(k)||$(l).data(k,[]),$(l).data(j)||$(l).data(j,[]),$(l).on(j,{componentDivId:l,bindName:j,eventName:h,triggerArrayName:k},function(a,b){var c=$(a.data.componentDivId).data(a.data.bindName),d;for(d in c){var e=c[d];b==e.flagMap[a.data.eventName].bindInterface&&e.notifyEvent(a.data.eventName)}ContextPlayer.executeTriggerArray($(a.data.componentDivId).data(a.data.triggerArrayName))})),
n.triggerBuffer=$(l).data(k),$(l).data(j).push(n)):(currentAssessment=q[m.role],currentAssessment.bindComponent=j,currentAssessment.bindInterface=l,currentAssessment.variable&&(localParamMap=ContextPlayer.createLocalParamMap(m.bindParam),currentAssessment.value=localParamMap[currentAssessment.value]?localParamMap[currentAssessment.value]:p[currentAssessment.value]))}};MediaPlayer.prototype=new Player;function MediaPlayer(a,b){this.htmlPlayerBkg=this.htmlPlayer=this.type=this.region="";this.popcornPlayer={};this.area=[];this.transIn=[];this.transOut=[];this.isPlaying=!1;this.isVisible=this.isStopped=!0;this.isFocused=!1;this.opacity=1;this.node=a;this.presentation=b.presentation;this.parentContext=b;this.create(a)}MediaPlayer.prototype.checkType=function(a){var b=this.type.split("/"),c;for(c in a){var d=a[c].split("/");if(d[0]==b[0]&&(!d[1]||b[1]&&d[1]==b[1]))return!0}return!1};
...
"presentation","attribution"];null!=a.eventType&&-1==jQuery.inArray(a.eventType,values)&&Debugger.error(Debugger.ERR_INVALID_ATTR_VALUE,b,["eventType",a.eventType,values]);values="0-9,A-Z,*,#,MENU,INFO,GUIDE,CURSOR_DOWN,CURSOR_LEFT,CURSOR_RIGHT,CURSOR_UP,CHANNEL_DOWN,CHANNEL_UP,VOLUME_DOWN,VOLUME_UP,ENTER,RED,GREEN,YELLOW,BLUE,BACK,EXIT,POWER,REWIND,STOP,EJECT,PLAY,RECORD,PAUSE".split(",");patt=/^([0-9A-Z]|\*|#|MENU|INFO|GUIDE|CURSOR_DOWN|CURSOR_LEFT|CURSOR_RIGHT|CURSOR_UP|CHANNEL_DOWN|CHANNEL_UP|VOLUME_DOWN|VOLUME_UP|ENTER|RED|GREEN|YELLOW|BLUE|BACK|EXIT|POWER|REWIND|STOP|EJECT|PLAY|RECORD|PAUSE)$/;
...
values=["presentation","selection","attribution"];null!=a.eventType&&-1==jQuery.inArray(a.eventType,values)&&Debugger.error(Debugger.ERR_INVALID_ATTR_VALUE,b,["eventType",a.eventType,values]);values="0-9,A-Z,*,#,MENU,INFO,GUIDE,CURSOR_DOWN,CURSOR_LEFT,CURSOR_RIGHT,CURSOR_UP,CHANNEL_DOWN,CHANNEL_UP,VOLUME_DOWN,VOLUME_UP,ENTER,RED,GREEN,YELLOW,BLUE,BACK,EXIT,POWER,REWIND,STOP,EJECT,PLAY,RECORD,PAUSE".split(",");patt=/^([0-9A-Z]|\*|#|MENU|INFO|GUIDE|CURSOR_DOWN|CURSOR_LEFT|CURSOR_RIGHT|CURSOR_UP|CHANNEL_DOWN|CHANNEL_UP|VOLUME_DOWN|VOLUME_UP|ENTER|RED|GREEN|YELLOW|BLUE|BACK|EXIT|POWER|REWIND|STOP|EJECT|PLAY|RECORD|PAUSE)$/;
