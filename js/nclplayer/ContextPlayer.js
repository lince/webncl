		...
		case "context": {
			...
			if (!this.context[port.component.id]) {
				...
				if (!this.context[port.component.id].port[port.nodeInterface.id]) {
					Debugger.error(Debugger.ERR_INVALID_CONTEXT_REFERENCE,"port",[port.id,"nodeInterface",port.nodeInterface.id]);
				} else {
...
ContextPlayer.createLocalParamMap = function (bindParam) {
	...
	for (i in bindParam) {
		...
		map['$'+currentBindParam.name.name] = currentBindParam.value;
	}
	return map;
};
...
ContextPlayer.prototype.bindLinks = function()
{
	...
	for ( i in links)
	{
		...
		for (var i in link.bind)
		{
			...
			var currentInterface = currentBind.nodeInterface? ( currentBind.nodeInterface.id || currentBind.nodeInterface.name ) : undefined;
