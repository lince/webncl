// <simpleAction>
function debugSimpleAction (obj,debug,parent,tree) {
	// role
	...
	if (obj.actionType!=null && jQuery.inArray(obj.actionType,values)==-1) {
		debug.error(this.ERR_INVALID_ATTR_VALUE,tag,["actionType",obj.actionType,values]);
	}
	// eventType
	...
	if (obj.value==null) {
		if (obj.eventType=="attribution" || obj.role=="set") {
			debug.error(debug.ERR_MISSING_ATTR,tag,["value"]);
		}
	}
	// min
	...
	if (obj.repeat!=null && !patt.test(obj.repeat)) {
		debug.error(this.ERR_INVALID_ATTR_VALUE,tag,["repeat",obj.repeat,values]);
	}
	// duration
	values = ["(número inteiro positivo)s"];
	patt = /^\s*\d+\s*s\s*$/;
	if (obj.duration!=null && !patt.test(obj.duration)) {
		debug.error(this.ERR_INVALID_ATTR_VALUE,tag,["duration",obj.duration,values]);
	}
}